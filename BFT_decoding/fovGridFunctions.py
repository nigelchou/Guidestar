...
def _stageToImageCoords(stage_to_pixel_matrix: np.ndarray,
    ...
    if stage_coords.shape != (2,):
        raise ValueError(f"invalid stage coords {stage_coords}: "
...
def _divideCanvas(image_coord_dict: dict,
    ...
    assume that most FOVs in a row/column should be separated by at least 200 pixels (min_separation parameter)
    (actually the smallest we have is 1024 pix for Nemo, up to 2960 for triton 1/2 microscopes),
    but would be offset from one another by less than 200 pixels along the orthogonal axis
    (ususally the are almost perfectly lined up)
