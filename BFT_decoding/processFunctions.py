...
from BFT_decoding.utils.registrationFunctions import register_translation
...
def registerPhaseCorr(imgdata: ImageData,
    ...
    register_by_hyb: bool
        if False, register all bits independently
        if True, all bits of the same hyb are assumed to be registered
    reference_bit: int,
    ...
    if register_by_hyb:
        assert reference_bit == 0, (
            "register_by_hyb requires reference bit to be 0. It currently only works with smFISH callouts")
        registration_shifts_hyb_dict = {}
        registration_error_hyb_dict = {}
        for hyb in set(hyb_list[1:]):
            bits = [i for i in range(len(hyb_list[1:])) if hyb_list[i]==hyb]
            if len(bits) == 1:
                imgs_hyb_max_projection = reg_array[0, :, :, bits[0]]
            elif len(bits) > 1:
                imgs_hyb_max_projection = reg_array[0, :, :, bits].max(0)
            
            (shifts,
             fine_error,
             pixel_error) = register_translation(reg_array[0,:,:,reference_bit],
                                                 imgs_hyb_max_projection,
                                                 upsample_factor=upsampling)
            registration_shifts_hyb_dict[hyb] = shifts           
            registration_error_hyb_dict[hyb] = {"fine error": fine_error,
        ...
        for bit in valid_bits:
            ...
            if bit == reference_bit:
                registration_shifts[bit] = np.zeros(ndims_register)
                registration_error_dict[bit] = {"fine error": None,
                                                "pixel error": None}
            else:
                registration_shifts[bit] = registration_shifts_hyb_dict[hyb_list[bit]]
                registration_error_dict[bit] = registration_error_hyb_dict[hyb_list[bit]]
            ...
            if align_images_after_estimation:
                ...
                registered_slice = np.fft.ifftn(
                    ndimage.fourier_shift(current_slice_fourier, registration_shifts[bit])
                )
