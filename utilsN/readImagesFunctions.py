...
def readImages(img_list: List[tuple],
    ...
    print("IMAGELLIST", img_list)

    for img_num in range(num_imgs):
        
        colour_list.append(img_list[img_num][1])
        
        if img_num not in bits_to_drop:
            
           print("Not drop", img_list[img_num][1])
        ...
        #  Trition : tuples of (filename, type/colour, frame of tiff)
           ...
           if microscope_type in ["Dory", "Nemo"]:
           ...
           elif microscope_type in ["Triton", "confocal"]:  # uses .tif file format
            ...
            # elif microscope_type in ["ConfocalZ"]:  # uses .tif file format but with z stack - JB
        ...
        if img_num in bits_to_drop:
           
           s= (xdim,ydim)
          # temp_image = np.zeros(s) 
           temp_image = np.ones(s) 
           print("Drop bit ")
           print(f"shape of image {img_num} is {temp_image.shape}.\n")
