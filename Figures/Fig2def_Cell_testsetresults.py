...
GS_filepath = 'Cell_LM39_25FOVs_GuidestarGenes_training.csv'
merfish_filepath = 'Cell_LM39_25FOVs_MerfishData.csv'
...
results_df.to_csv(os.path.join(output_dir,'testset_results_cell.csv'))
...
merfish_results_df.to_csv(os.path.join(output_dir, 'merfish_results_cell.csv'))
...
results_df = pd.read_csv(os.path.join(output_dir,'testset_results_cell.csv'))
merfish_results_df = pd.read_csv(os.path.join(output_dir, 'merfish_results_cell.csv'))
...
fig.tight_layout()
fig.savefig(os.path.join(output_dir, 'testset_results_cell.svg'))
...
axes[0].set_ylim(0,1)
axes[1].set_ylim(0,70000) # cell

axes[0].set_title('fpkm')
axes[1].set_title('counts')
fig.tight_layout()
fig.savefig(os.path.join(output_dir, 'merfish_results_cell_withwhole.svg'))
...
for j,test_fov in enumerate(testing_fovs):
    ...
    # plt.plot(fpr_model,tpr_model,c='tab:blue')
    # plt.plot(fpr_bft,tpr_bft,c='tab:orange')
    # plt.plot(fpr_bft[np.abs(thresholds_bft-(1-bft)).argmin()],tpr_bft[np.abs(thresholds_bft-(1-bft)).argmin()],'x')
    ...
    # aucs_model.append(roc_model.roc_auc)
    ...
    # aucs_bft.append(roc_bft.roc_auc)
...
mean_tpr_bft[-1]=1
ax1.plot(mean_fpr,mean_tpr_model,color='tab:blue', linewidth=2, label='Guidestar')
ax1.plot(mean_fpr,mean_tpr_bft,color='tab:orange', linewidth=2, label='bft')
ax1.legend(loc='lower right')
ax1.set_xlabel('false positive rate')
ax1.set_ylabel('true positive rate')
fig1.savefig(os.path.join(output_dir, 'ROC_withmisid.svg'))
...
correlation_plot = plotScatter(ax=ax, df=correlation_df, x_column='fpkm', y_column='Guidestar', y_column2='BFT', fontprops=results_fontprops, ylim_offset=10)
ax.set_title('model corr: ' + str(np.round(model_fpkm_val,3)) + ' bft corr: ' + str(np.round(bft_fpkm_val,3)))
fig.savefig(os.path.join(output_dir, 'Cell_correlationwithfpkm.svg'))